...
const logger$2 = LogManager.get().getLogger(), OfflineDatabaseObjectListProperty = asProperty((config, store, widgetId, propertyPath) => {
  const queryArguments$ = Object.fromEntries(Object.entries(config.arguments ?? {}).map(([runtimeName, [source, _, isLocalVariable]]) => {
    const name = ensure(runtimeName.split("$")[0]);
    return tuple(name, isLocalVariable ? localVariableRuntimeValue$(name, { widget: source, source: "localvariable" }, store) : getSubscribedObjectState$(store, source));
  })), constraints = config.constraints, fetchParameters$ = constraints ? fromComputation$(`queryFilter for ${widgetId}/${propertyPath}`, () => {
